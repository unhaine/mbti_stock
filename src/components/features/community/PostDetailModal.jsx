...
export default function PostDetailModal({ post, isOpen, onClose }) {
  ...
  // 가상 댓글
  const comments = [
    { author: '투자고수', mbti: 'INTJ', content: '좋은 정보 감사합니다! 저도 비슷하게 생각하고 있었어요.', timeAgo: '10분 전' },
    { author: '주린이', mbti: 'ENFP', content: '오 대박! 저도 참고해볼게요 🙌', timeAgo: '25분 전' },
    { author: '차트분석가', mbti: 'ISTP', content: '기술적으로도 좋아보이네요.', timeAgo: '1시간 전' },
  ]

  // 카테고리별 본문 내용 매핑
  const getCategoryContent = (category) => {
    const contentMap = {
      '수익 인증': '오늘 드디어 목표했던 수익률을 달성했어요! 꾸준히 분석하고 기다린 보람이 있네요. 모두들 화이팅!',
      '종목 토론': '이 종목에 대해 여러분의 생각이 궁금해요. 펀더멘털도 좋고 차트도 좋아보이는데, 다양한 관점에서 의견 나눠주세요!',
      '투자 토론': '투자 스타일에 대해 많이 고민하게 됩니다. 장기 투자의 안정감도 좋지만, 단타의 짜릿함도 있잖아요?',
      '첫 투자': '드디어 첫 주식을 매수했어요! 떨리기도 하고 설레기도 하고... 선배님들의 조언 부탁드립니다 🙏',
      '테마 분석': '요즘 이 테마가 정말 핫한 것 같아요. 관련 종목들 정리해봤는데 같이 공유드립니다!',
      ...
      'MBTI 투자': '제 MBTI에 딱 맞는 투자 스타일을 찾은 것 같아요! 역시 성격별 투자가 맞는 것 같습니다 ㅎㅎ',
      '투자 팁': '제가 수익을 낼 수 있었던 비결을 공유합니다. 완전 특별한 건 아니지만 기본에 충실한 게 답인 것 같아요.',
        ...
        >
          ...
          <div className="h-full flex flex-col">
            ...
            <motion.div
              ...
              className="flex-shrink-0 px-4 py-3 flex items-center justify-between border-b border-dark-600 bg-white/95 backdrop-blur-sm"
            >
              <button
                onClick={onClose}
                className="p-2 -ml-2 rounded-full hover:bg-dark-700 text-dark-400 hover:text-white transition-colors"
              >
              ...
              <button className="p-2 -mr-2 rounded-full hover:bg-dark-700 text-dark-400 hover:text-white transition-colors">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <span className="text-dark-400 text-sm">{post.timeAgo} • 조회 {post.views}</span>
                    </div>
                  </div>
                </motion.div>
                        ...
                        <div key={i} className="flex gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-medium text-dark-50 text-sm">{comment.author}</span>
                              <span className="text-xs text-dark-500">{comment.mbti}</span>
                              <span className="text-xs text-dark-600">•</span>
                              <span className="text-xs text-dark-500">{comment.timeAgo}</span>
                            </div>
                            <p className="text-dark-300 text-sm leading-relaxed">{comment.content}</p>
                          </div>
                        </div>
                      )
                    })}
            ...
            <motion.div
              ...
              className="flex-shrink-0 px-4 py-3 border-t border-dark-600 bg-white/95 backdrop-blur-sm"
            >
